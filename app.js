...
function renderHome(app){
  ...
  app.innerHTML=`
    <section class="hero minimal-home">
      ...
      <div class="home-sequence" aria-label="Case workflow"><span>Examine</span><i>→</i><span>Frame</span><i>→</i><span>Clarify</span><i>→</i><span>Update</span><i>→</i><span>Finalize</span></div>
...
function workflowHTML(){
  ...
  else if(!clarified) active=1;
  else if(hasNewData()) active=2;
  else active=1;
  const steps=[
...
function actionButtonHTML(a,c){
  const used=!a.repeatable&&S.actionsTaken.includes(a.id); const allowed=!Array.isArray(c.availableActions)||c.availableActions.includes(a.id); const available=allowed && (a.id!=="nihss" || (c.strokeTools||[]).includes("nihss")) && (Object.prototype.hasOwnProperty.call(c.r,a.id) || a.id==="reexam");
  ...
  return `<button class="action-card ${used?'used':''}" data-action="${a.id}" ${used||!available?'disabled':''}><b>${htmlSafe(label)}</b>${used?'<small>Already reviewed</small>':(purpose?`<small>${htmlSafe(purpose)}</small>`:'')}</button>`;
}
...
function openQuestionBeforeAction(id,q){
  ...
  showSheet("",q.prompt||"What question are you trying to answer?",`
  ...
  $("#sheetBody").querySelectorAll("[data-qopt]").forEach(b=>b.onclick=()=>{
    ...
    $("#questionFeedback").innerHTML=`<div class="active-feedback"><p>${feedback}</p></div>`;
...
function openModelSheet(first){
  const c=getCase(), r=reasoningFor(c); if(!c)return;
  const paces=(r.tempoOptions||TEMPOS.filter(x=>x.id!=='acute_on_subacute').map(x=>x.id)).map(id=>TEMPOS.find(x=>x.id===id)).filter(Boolean);
  ...
  let step=0;
  const steps=[
    {title:'What is the pace?',help:'Use the onset and evolution of symptoms—not the total duration of illness.',body:()=>`<div class="choice-list">${paces.map(x=>radioHTML("tempo",x.id,x.title,x.desc,S.tempo)).join('')}</div>`},
    {title:'Where does this localize?',help:'Use the history and raw neurologic examination before naming a diagnosis.',body:()=>`<div class="choice-list">${locs.map(x=>radioHTML("localization",x.id,x.title,x.desc,S.localization)).join('')}</div>`},
    {title:'What syndrome does this define?',help:'Combine pace and localization into a clinical syndrome before choosing an etiology.',body:()=>`<div class="choice-list">${syns.map(x=>radioHTML("syndrome",x.id,x.title,x.desc,S.syndrome)).join('')}</div>`},
    {title:'Now add context. What etiologies fit?',help:'Use the patient’s age, history, setting, exposures, and comorbidities to order the differential. Context should modify the syndrome, not replace it.',body:()=>probBoardHTML(r.hypotheses,S.hypothesisLevels,"modelprob")}
  ];
  function draw(){
    const x=steps[step];
    showSheet(first?'Frame the problem':'Revise the model',x.title,`<div class="frame-progress"><span>${step+1} of ${steps.length}</span><div>${steps.map((_,i)=>`<i class="${i<=step?'on':''}"></i>`).join('')}</div></div>${step<3?'<p class="frame-principle">Define the syndrome first. Use clinical context when you build the differential.</p>':''}<p class="section-help">${x.help}</p>${x.body()}<div id="modelWarning" class="note warning" hidden></div><div class="sheet-actions"><button class="secondary" id="frameBack" ${step===0?'disabled':''}>Back</button><button class="primary" id="frameNext">${step===steps.length-1?'Save working model':'Next'}</button></div>`);
...
function openUpdateSheet(){
  ...
  showSheet("","Update model",`<div class="update-frame-link"><button class="secondary" id="reviseFrame">Revise pace / localization / syndrome</button></div>${rows}<div class="sheet-actions"><button class="secondary" id="cancelUpdate">Close</button><button class="primary" id="saveUpdate">Save update</button></div>`);
...
function renderDebrief(app,c){
  ...
  const frameRows=[
    S.tempo?`<div><span>Pace</span><b>${htmlSafe(titleFor(TEMPOS,S.tempo))}</b></div>`:'',
    S.localization?`<div><span>Localization</span><b>${htmlSafe(titleFor(LOCALIZATIONS,S.localization))}</b></div>`:'',
    S.syndrome?`<div><span>Syndrome</span><b>${htmlSafe(titleFor(SYNDROMES,S.syndrome))}</b></div>`:''
  ].join('');
  app.innerHTML=`<section class="debrief"><div class="case-kicker">Case ${c.n}</div><h1>Debrief</h1>
    ${frameRows?`<div class="debrief-section"><h2>Reasoning frame</h2><div class="final-summary reasoning-frame-summary">${frameRows}</div></div>`:''}
    ...
    <div class="debrief-section"><h2>${htmlSafe(c.debriefDecisionTitle||"Key reasoning")}</h2><ul class="plain-list">${c.teach.map(t=>`<li>${t}</li>`).join('')}</ul></div>
